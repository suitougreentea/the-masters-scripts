...
const createKeepAliveResponse = (onOpen: () => void, onClose: () => void): Response => {
  const encoder = new TextEncoder();
  let id = -1;
  const responseBody = new ReadableStream({
    start(controller) {
      onOpen();
      id = setInterval(() => controller.enqueue(encoder.encode("data: keepalive\n\n")), 1000);
    },
    cancel() {
      onClose();
      clearInterval(id);
      ...
      async (req) => {
        ...
        switch (url.pathname) {
          ...
          case "/register/updatePlayer":
            {
              try {
                const { oldName, playerEntry } = await req.json() as UpdatePlayerArgs;
