...
export class MastersTabsElement extends LitElement {
  ...
  render() {
    return html`
      ...
      <fluent-tabs class="container" orientation="vertical" @change="${this
        ._onTabChange}">
        <fluent-tab slot="tab" id="setup">セットアップ</fluent-tab>
        ${map(this._roundNames, (e, i) =>
          html`
            <fluent-tab slot="tab" id="round${i}">${e}</fluent-tab>
          `)}
      ...
      <span class="end">
        <div class="end-bar">
          <fluent-button
            ...
            @click="${this._confirmFinishCompetitionWithExport}"
          >大会を終了</fluent-button>
        ...
        <fluent-menu id="advanced-menu" class="menu">
          <fluent-menu-item
            .disabled="${!this._hasMetadata}"
            @click="${this._confirmFinishCompetitionWithoutExport}"
          >エクスポートせずに大会を終了</fluent-menu-item>
          <fluent-menu-item
            @click="${this._reloadCompetitionMetadata}"
          >大会設定を再読み込み</fluent-menu-item>
        </fluent-menu>
      </span>
    `;
  }
