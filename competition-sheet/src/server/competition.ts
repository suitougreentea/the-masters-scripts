...
namespace Competition {
  ...
  export type StageResultEntryStub = Omit<StageResultEntry, "rank" | "timeDiffTop" | "timeDiffPrev">;
  type QualifierResultEntryStub = Omit<QualifierResultEntry, "rank">;
  type SupplementComparisonEntryStub = Omit<SupplementComparisonEntry, "rank" | "timeDiffPrev">;
  ...
  function setupCompetitionManual(name: string, numGames: number): CompetitionSetupResult {
    const rounds: RoundMetadata[] = new Array(numGames).fill(null).map((_, i) => ({
      name: String(i + 1),
      roundIndex: i,
      stages: [{
        name: String(i + 1),
  ...
  function setupCompetitionWithQualifier(name: string, preset: Preset.Preset, numPlayers: number): CompetitionSetupResult {
    // ポイント制予選
    ...
    firstRound.qualifierPlayerIndices.forEach((indices, i) => qualifierStages.push({
  ...
  function setupCompetitionWithTournament(name: string, preset: Preset.Preset, numOverallPlayers: number): CompetitionSetupResult {
    // トーナメント
    ...
    preset.rounds.forEach((round, roundIndex) => {
      ...
      for (let i = 0; i < round.numGroups!; i++) {
        const stageName = round.numGroups == 1 ? round.name : `${round.name} ${groupIndexToString(i)}組`;
      ...
      if (round.winners != null) {
        ...
        const numWildcard = hasWildcard ? 1 : 0;
        if (numWildcard > 0) {
          const unevenNumWinners = stages.some(e => e.numWinners != numMaxWinners);
          const comparisonName = unevenNumWinners ? `${round.name} ワイルドカード` : `${round.name} ${numMaxWinners + 1}位 (ワイルドカード)`;
          supplementComparisons.push({
            rankId: "W",
            name: comparisonName,
      ...
      if (round.losers != null) {
        if (round.losers.destinationMethod == "standard") {
          ...
          if (round.numGroups! > 1) {
            ...
            const unevenNumWinners = stages.some(e => e.numWinners != numWinners);
            const hasWildcard = stages[0].hasWildcard;
            if (stages.some(e => e.hasWildcard != hasWildcard)) throw new Error(); // 負けを並べるとき、ワイルドカード有無が揃ってない場合は未対応
            const numWildcard = hasWildcard ? 1 : 0;
            const numMaxLosers = Math.max(...stages.map(e => e.numLosers));
            const unevenNumLosers = stages.some(e => e.numLosers != numMaxLosers);
            if (!unevenNumWinners) {
              ...
              for (let i = 0; i < numMaxLosers; i++) {
                ...
                if (numRankPlayers == 1) continue;
                const comparisonName = i == numMaxLosers - 1 ? `${round.name} 最下位` : `${round.name} 下から${numMaxLosers - i}位`;
                supplementComparisons.push({
      ...
      const winnersDestination: DestinationInfo | undefined = round.winners != null ? {
  ...
  export function getDependencyForRound(metadata: CompetitionMetadata, roundIndex: number): RoundDependencyDefinition[] {
    ...
    if (roundIndex == 0) {
      ...
      if (metadata.type == "qualifierFinal") {
        ...
        const dependentRoundIndices = getDependentRoundIndicesFromMetadata(metadata, roundIndex);
        dependentRoundIndices.forEach(dependentRoundIndex => dependency.push({ type: "tournamentRoundResult", roundIndex: dependentRoundIndex }));
        return dependency;
      }
    }
    throw new Error();
   ...
   */
  export function setupRound(metadata: CompetitionMetadata, roundIndex: number, dependency: RoundDependencyData[]): RoundSetupResult {
    if (roundIndex == 0) {
      if (metadata.type == "qualifierFinal") {
      ...
      if (metadata.type == "qualifierFinal") {
        ...
        return setupSubsequentRoundWithTournament(metadata, roundIndex, dependency);
      }
    }
    throw new Error();
  }
   ...
   */
  export function validateEntriesWithQualifier(entries: Participant[], numPlayers: number) {
    entries.forEach((e) => {
  ...
  function setupFirstRoundWithQualifier(metadata: CompetitionMetadata, roundIndex: number, dependency: RoundDependencyData[]): RoundSetupResult {
    // ポイント制予選
    ...
    const stages: StageSetupResult[] = firstRound.stages.map(stageMetadata => {
      ...
      const stageEntries: StageSetupPlayerEntry[] = playerIndices.map(i => {
        const foundEntry = entryDependency.data.find(e => e.firstRoundGroupIndex == i);
  ...
  function setupFinalRoundWithQualifier(metadata: CompetitionMetadata, roundIndex: number, dependency: RoundDependencyData[]): RoundSetupResult {
    // ポイント戦決勝
    if (roundIndex != 1) throw new Error();
    ...
    const qualifierDependency = dependency.find(e => e.type == "qualifierRoundResult");
    ...
    for (let i = 0; i < 4; i++) {
      ...
      stages[0].entries.push({ name: qualifierResultEntry.name, handicap: assignHandicap(i) });
    }
   ...
   * @param numPlayers 現在の大会で想定されている参加人数
   */
  export function validateEntriesWithTournament(entries: Participant[], numGroups: number) {
    entries.forEach((e) => {
      if (e.firstRoundGroupIndex == null) {
        throw new Error("1回戦組が設定されていないプレイヤーがいます: " + e.name);
      }
  ...
  function setupFirstRoundWithTournament(metadata: CompetitionMetadata, roundIndex: number, dependency: RoundDependencyData[]): RoundSetupResult {
    // トーナメント1回戦
    if (roundIndex != 0) throw new Error();

    const firstRound = metadata.rounds[0];
    const stages: StageSetupResult[] = firstRound.stages.map(_ => ({ entries: [] }));
  ...
  function setupSubsequentRoundWithTournament(metadata: CompetitionMetadata, roundIndex: number, dependency: RoundDependencyData[]): RoundSetupResult {
    // トーナメント
    if (roundIndex == 0) throw new Error;

    const round = metadata.rounds[roundIndex];

    type StagePlayerEntryStub = { name: string, previousRoundRankIndex: number, previousRoundHandicapMethod: HandicapMethod };
    ...
    const sendPlayers = (dependentRoundIndex: number, winner: boolean) => {
      ...
      const destinationMethod = winner ? dependentRoundMetadata.winnersDestination!.method : dependentRoundMetadata.losersDestination!.method;

      const foundDependency = dependency.find(e => e.type == "tournamentRoundResult" && e.roundIndex == dependentRoundIndex);
      ...
      if (destinationMethod == "standard") {
        ...
        if (winner) {
          const numMaxWinners = Math.max(...dependentStageMetadata.map(e => e.numWinners));
          ...
          for (let i = 0; i < numMaxWinners; i++) {
            ...
            sortedResultsPerRank.forEach(result => {
              sortedStubsToPut.push({
                name: result.name,
                previousRoundHandicapMethod: dependentRoundMetadata.winnersDestination!.handicap,
          ...
          if (dependentStageMetadata.some(e => e.hasWildcard != hasWildcard)) throw new Error(); // ワイルドカード有無が揃ってない場合は未対応
          if (hasWildcard) {
          ...
          const unevenNumWinners = dependentStageMetadata.some(e => e.numWinners != numWinners);
          const hasWildcard = dependentStageMetadata[0].hasWildcard;
          if (dependentStageMetadata.some(e => e.hasWildcard != hasWildcard)) throw new Error(); // 負けを並べるとき、ワイルドカード有無が揃ってない場合は未対応
          const numWildcard = hasWildcard ? 1 : 0;
    ...
    dependents.forEach(dependentRoundIndex => {
      ...
      if (dependentRound.winnersDestination != null && dependentRound.winnersDestination.roundIndex == roundIndex) {
        sendPlayers(dependentRoundIndex, true);
      }
      if (dependentRound.losersDestination != null && dependentRound.losersDestination.roundIndex == roundIndex) {
        sendPlayers(dependentRoundIndex, false);
      }
    });
  ...
  export function finalizeRound(metadata: CompetitionMetadata, roundIndex: number, stageResults: StageResultEntry[][]): RoundFinalizeResult {
  ...
  function finalizeQualifierRound(metadata: CompetitionMetadata, roundIndex: number, stageResults: StageResultEntry[][]): RoundFinalizeResult {
    if (roundIndex != 0) throw new Error();
    ...
    if (qualifierRound.stages.some(e => e.fixedPlayerIndices == null || e.fixedPlayerIndices.length != numPlayersPerGroup)) throw new Error();

    // 全部のリザルトが揃っていることを確認
    if (stageResults.some(stageResult => stageResult.length != numPlayersPerGroup)) throw new NotReadyError();

    // プレイヤーごとの情報
    const qualifierPlayerData: { name: string, participatingStageResults: StageResultEntry[] }[] = [];
    ...
    const qualifierResultStubs: QualifierResultEntryStub[] = qualifierPlayerData.map(e => {
      let points = 0;
      ...
      const bestResult = e.participatingStageResults[e.participatingStageResults.length - 1];
  ...
  function finalizeTournamentRound(metadata: CompetitionMetadata, roundIndex: number, stageResults: StageResultEntry[][]): RoundFinalizeResult {
    ...
    stageResults.forEach((stageResult, stageIndex) => {
      if (stageResult.length != roundMetadata.stages[stageIndex].numPlayers) throw new NotReadyError();
    });
  ...
  export function overallStageIndexToRoundGroupIndex(metadata: CompetitionMetadata, overallStageIndex: number): { roundIndex: number, groupIndex: number } | null {
    let startIndex = 0;
    for (let i = 0; i < metadata.rounds.length; i++) {
      const round = metadata.rounds[i];
      const numGroups = round.stages.length;
      if (startIndex <= overallStageIndex && overallStageIndex < startIndex + numGroups) {
  ...
  export function roundGroupIndexToOverallStageIndex(metadata: CompetitionMetadata, roundGroup: { roundIndex: number, groupIndex: number }): number | null {
  ...
  export function isQualifierRound(metadata: CompetitionMetadata, roundIndex: number) {
    return metadata.type == "qualifierFinal" && roundIndex == 0;
  }

  function getDependentRoundIndicesFromPreset(preset: Preset.Preset, roundIndex: number): number[] {
    const result: number[] = [];
  ...
  function getDependentRoundIndicesFromMetadata(metadata: CompetitionMetadata, roundIndex: number): number[] {
    const result: number[] = [];
    metadata.rounds.forEach((e, i) => {
      if (e.winnersDestination != null && e.winnersDestination.roundIndex == roundIndex) result.push(i);
      if (e.losersDestination != null && e.losersDestination.roundIndex == roundIndex) result.push(i);
    });
    return result;
  }
  ...
  function compareQualifierScore(a: QualifierResultEntryStub, b: QualifierResultEntryStub): number {
   ...
   * @returns
   */
  export function getStageResult(scores: StageResultEntryStub[]): StageResultEntry[] {
    const sorted = [...scores];
  ...
  function getSupplementComparison(scores: SupplementComparisonEntryStub[]): SupplementComparisonEntry[] {
    const sorted = [...scores];
    sorted.sort(compareStageScore).reverse();
  ...
  function getQualifierResult(scores: QualifierResultEntryStub[]): QualifierResultEntry[] {
    const sorted = [...scores];
