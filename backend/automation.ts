import { Participant, StagePlayerEntry, StageScoreData, StageScoreEntry, StageSetupPlayerEntry, StageSetupResult } from "../common/common_types.ts";
...
export const resolvePresetName = (numPlayers: number, inputPresetName?: string): string => {
...
export const populateParticipants = (numPlayers: number, presetName?: string): Participant[] => {
  const preset = presetName != null ? getPreset(presetName) : null;
  const manual = preset == null;

  const playersStoreValue = getSerializerValue("players") as { registeredPlayers: { name: string }[] };
  const playerNames = playersStoreValue.registeredPlayers.map(e => e.name).slice(0, numPlayers);
...
export const populateCompetitionSetupOptions = (numPlayers: number, presetName?: string): CompetitionSetupOptions => {
...
export const populateStageSetupResultFromParticipants = (participants: Participant[], numPickPlayers: number): StageSetupResult => {
...
export const populatePlayerReorderList = (players: (StagePlayerEntry | null)[]): (string | null)[] => {
  const names = players.map(e => e != null ? e?.name : null);
  shuffle(names);
  return names;
};

export const populateStageScore = (players: (StagePlayerEntry | null)[]): StageScoreData => {
  const score = () => {
    if (Math.random() < 0.4) {
...
export const automateWholeCompetition = async (numPlayers: number, manual: boolean, presetName?: string, exportToSpreadsheet?: boolean): Promise<{ exportUrl?: string }> => {
  loadTestDataInMemory();

  const resolvedPresetName = manual ? undefined : resolvePresetName(numPlayers, presetName);
  const participants = populateParticipants(numPlayers, resolvedPresetName);
  Api.mastersSetParticipants(participants);
  const competitionSetupOptions = populateCompetitionSetupOptions(numPlayers, resolvedPresetName);
  ...
  metadata.rounds.forEach((round, roundIndex) => {
    ...
    round.stages.forEach((_, stageIndex) => {
      if (manual) {
        Api.mastersResetStage(roundIndex, stageIndex, populateStageSetupResultFromParticipants(participants, 4));
      }

      const stageData = Api.mastersGetStageData(roundIndex, [stageIndex])[0];
      Api.mastersReorderStagePlayers(roundIndex, stageIndex, populatePlayerReorderList(stageData.players));

      const newStageData = Api.mastersGetStageData(roundIndex, [stageIndex])[0];
      Api.mastersSetStageScore(roundIndex, stageIndex, populateStageScore(newStageData.players));
    });
