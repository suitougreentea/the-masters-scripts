...
export const createStagesSheet = (ss: GoogleAppsScript.Spreadsheet.Spreadsheet, input: StagesData, templatesSheet: GoogleAppsScript.Spreadsheet.Sheet): GoogleAppsScript.Spreadsheet.Sheet => {
  const { list } = input;

  const sh = ss.insertSheet("Competition");

  resizeSheet(sh, 11 * list.length - 1, 17);
  setColumnWidths(sh, [16, 80, 70, 40, 70, 24, 70, 40, 70, 30, 16, 80, 40, 70, 70, 70, 70]);

  let baseRow = 1;
  list.forEach(stage => {
    const { name, entries, result, borderlines } = stage;
    pasteTemplate(sh, baseRow, 1, templatesSheet, getCompetitionStageTemplate(), SpreadsheetApp.CopyPasteType.PASTE_NORMAL);
    ...
    entries.forEach((e, i) => {
      ...
      if (1 <= e.startOrder && e.startOrder <= 4) {
        nameRange.setBackground("black");
      }
      if (5 <= e.startOrder && e.startOrder <= 8) {
        nameRange.setBackground("red");
      }
    })
    // TODO: color by startOrder
