...
export const createParticipantsSheet = (ss: GoogleAppsScript.Spreadsheet.Spreadsheet, input: ParticipantsData): GoogleAppsScript.Spreadsheet.Sheet => {
  const { list } = input;
  ...
  numPlayersHeaderRange.setBackground("#000000").setFontColor("#FFFFFF").setFontWeight("bold");
  ...
  valueRange.setValues(list.map((e) => [e.name, groupIndexToString(e.firstRoundGroupIndex), timeToSpreadsheetValue(e.bestTime)]));
